...
#import "VVCallStack.h"


//为通用回溯设计结构支持栈地址由小到大，地址里存储上个栈指针的地址
typedef struct vv_stack_frame {
    const struct vv_stack_frame *const previous;
    const uintptr_t return_address;
} vv_stack_frame;

//存储 thread 信息的结构体
typedef struct vv_thread_info_struct {
    double cpuUsage;
    integer_t userTime;
} vv_thread_info_struct;

static mach_port_t _vv_main_thread_id;

@implementation VVCallStack

+ (void)load {
    _vv_main_thread_id = mach_thread_self();
}

#pragma mark - Interface

+ (NSString *)call_stack_with_type:(VVCallStackType)type {
    if (type == VVCallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:vv_stack_of_thread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
        ...
        return [reStr copy];
    } else if (type == VVCallStackTypeMain) {
        //主线程
        NSString *reStr = vv_stack_of_thread((thread_t) _vv_main_thread_id);
        assert(vm_deallocate(mach_task_self(), (vm_address_t) _vv_main_thread_id, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        NSLog(@"%@", reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (pt) {
                ...
                if (!strcmp(name, [nsthread name].UTF8String)) {
                    ...
                    reStr = vv_stack_of_thread(list[i]);
        ...
        [nsthread setName:originName];
        reStr = vv_stack_of_thread(mach_thread_self());
        NSLog(@"%@", reStr);
        return [reStr copy];
    }
    return @"";
...
//当前栈帧结构
NSString *vv_stack_of_thread(thread_t thread) {
    //thread info
    vv_thread_info_struct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = vv_thread_state_count_by_cpu();
    kern_return_t kr = thread_get_state(thread, vv_thread_state_by_cpu(), (thread_state_t) &machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = vv_mach_instruction_pointer_by_cpu(&machineContext);
    buffer[i] = instructionAddress;
    ++i;

    //
    uintptr_t linkRegisterPointer = vv_mach_thread_get_link_register_pointer_by_cpu(&machineContext);
    ...
    vv_stack_frame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = vv_mach_stack_base_pointer_by_cpu(&machineContext);
    if (framePointer == 0 || vv_mem_copy_safely((void *) framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || vv_mem_copy_safely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    vv_symbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", vv_output_log(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString *vv_output_log(const int entryNum, const uintptr_t address, const Dl_info *const dlInfo) {
...
kern_return_t vv_mem_copy_safely(const void *const src, void *const dst, const size_t byteSize) {
...
void vv_symbolicate(const uintptr_t *const stackBuffer, Dl_info *const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        vv_dladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        vv_dladdr(vv_instruction_address_by_cpu(stackBuffer[i]), &symbolsBuffer[i]);
    }
}

bool vv_dladdr(const uintptr_t address, Dl_info *const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = vv_dyld_image_index_from_address(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = vv_segment_base_of_image_index(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
...
uint32_t vv_dyld_image_index_from_address(const uintptr_t address) {
...
uintptr_t vv_cmd_first_pointer_from_mach_header(const struct mach_header *const machHeader) {
...
uintptr_t vv_segment_base_of_image_index(const uint32_t idx) {
    const struct mach_header *machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = vv_cmd_first_pointer_from_mach_header(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t vv_mach_stack_base_pointer_by_cpu(mcontext_t const machineContext) {
...
uintptr_t vv_mach_instruction_pointer_by_cpu(mcontext_t const machineContext) {
...
uintptr_t vv_instruction_address_by_cpu(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t vv_thread_state_count_by_cpu() {
#if defined(__arm64__)
...
*/
thread_state_flavor_t vv_thread_state_by_cpu() {
#if defined(__arm64__)
...
uintptr_t vv_mach_thread_get_link_register_pointer_by_cpu(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
