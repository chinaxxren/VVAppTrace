...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            ...
            vv_call_record *log = &_vv_call_records[_vv_record_num - 1];
            log->cls = p_record->cls;
            log->depth = curIndex;
            log->sel = p_record->cmd;
            log->time = cost;
            
            printf("trace->[%s %s] %6.2f\n",class_getName(log->cls),sel_getName(log->sel),log->time/1000.0f);
        }
    }
