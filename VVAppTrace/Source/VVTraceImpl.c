...
uintptr_t get_lr() {
    ...
    if (vv_print > 0) {
        ...
        if (interval > method_min_duration) {
            printf("%s %s %llu\n", (char *) ls->obj, (char *) ls->sel, interval);
...
void vv_min_duration(long duration) {
    method_min_duration = duration;
}

void vv_start(char *log_path) {
    vv_resume_print();
...
//    rebind_symbols((struct rebinding[1]){{"objc_msgSend", hook_objc_msgSend, (void *)&orig_objc_msgSend}}, 1);
    ...
    vv_rebind_symbols_image(header, slide, (struct rebinding[1]) {{"objc_msgSend", hook_objc_msgSend, (void *) &orig_objc_msgSend}}, 1);
}
